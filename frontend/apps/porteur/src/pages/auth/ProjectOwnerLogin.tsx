...
export default function ProjectOwnerLogin() {
	...
	const signInState: any = useSignIn();
	const signUpState: any = useSignUp();
	const signIn: any = signInState.signIn?.value || signInState.signIn;
	const signUp: any = signUpState.signUp?.value || signUpState.signUp;
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      const result = await signUp.create({
        ...
        password,
        unsafeMetadata: { role: "porteur", companyName: companyName.trim() || undefined },
      });
      if (result.status !== "complete" || !result.createdSessionId) {
        await signUp.verifications.prepareEmailAddress({ strategy: "email_code" });
        throw new Error("Un code de vérification a été envoyé par e-mail");
      }
      await clerk.setActive({ session: result.createdSessionId });
      window.dispatchEvent(new Event("zira-auth-ready"));
      navigate("/porteur/dashboard");
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-background p-4 relative">
      <div className="w-full max-w-md space-y-6">
        <div className="text-center space-y-2">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-emerald-600 text-white text-2xl font-black mb-2 shadow-sm">
          ...
          <p className="text-muted-foreground text-sm">
            {t("auth.projectOwnerPortalSubtitle", "Levez des fonds en toute conformité et accélérez votre croissance.")}
          </p>
        </div>
        ...
        <Card className="border-border shadow-md">
          <CardHeader className="pb-3">
            <CardTitle className="text-xl">
              {t("auth.projectOwnerPortal", "Espace Porteur de Projet")}
            </CardTitle>
            <CardDescription>
              {lang === "fr"
                ? "Connectez-vous pour piloter votre levée et vos relations investisseurs"
                : "Sign in to manage your fundraising campaigns and investor relations"}
            </CardDescription>
          </CardHeader>
            ...
            <Tabs value={tab} onValueChange={(v) => { setTab(v as "login" | "register"); setError(""); }}>
              ...
              <TabsContent value="login">
                <form onSubmit={handleLogin} className="space-y-4">
                  <div className="space-y-1.5">
                    <Label htmlFor="port-login-ident">{t("auth.identifier", "Email ou nom d'utilisateur")}</Label>
                    <Input
                      id="port-login-ident"
                      type="text"
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      <Label htmlFor="port-login-password">{t("auth.password", "Mot de passe")}</Label>
                      <button
                        type="button"
              ...
              <TabsContent value="register">
                <form onSubmit={handleRegister} className="space-y-4">
                  <div className="space-y-1.5">
                    <Label htmlFor="port-reg-username">{t("auth.username", "Nom d'utilisateur")}</Label>
                    <Input
                      id="port-reg-username"
                      type="text"
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="port-reg-email">{t("auth.email", "Adresse email")}</Label>
                    <Input
                      id="port-reg-email"
                      type="email"
                      placeholder="fondateur@startup.cd"
                      value={email}
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="port-reg-name">{t("auth.fullName", "Nom complet du fondateur")}</Label>
                    <Input
                      id="port-reg-name"
                      type="text"
                      placeholder="ex: Cédric Mpolo"
                      value={fullName}
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="port-reg-company">{t("profile.companyName", "Entreprise / Raison Sociale")}</Label>
                    <Input
                      id="port-reg-company"
                      type="text"
                      placeholder="ex: KaziPay Technologies SARL"
                      value={companyName}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-1.5">
                      <Label htmlFor="port-reg-password">{t("auth.password", "Mot de passe")}</Label>
                      <Input
                        id="port-reg-password"
                        type="password"
                    ...
                    <div className="space-y-1.5">
                      <Label htmlFor="port-reg-confirm">{t("auth.confirmPassword", "Confirmer")}</Label>
                      <Input
                        id="port-reg-confirm"
                        type="password"
                        placeholder="Min 8 car."
                  ...
                  <Button
                    ...
                    disabled={loading}
                  >
                    {loading ? t("common.loading", "Création...") : t("auth.register", "Créer mon compte porteur")}
                    <ArrowRightIcon className="w-4 h-4 ml-2" />
                  </Button>
                </form>
              </TabsContent>
